...
module HexunStock =
    ...
    let internal column =
        "Price,Open,High,Low,UpDown,UpDownRate,PE2,ExchangeRatio"
    ...
    let getStockData stock =
        async {
            let code = stock.Code
            let baseURL =
                sprintf "http://%s.%s/%s/quotelist" (getHostPrefix code)
                    baseHost (getRegion code)
            let url =
                sprintf "%s?code=%s&column=%s&callback=%s" baseURL
                    (getCodeString code) column callback
...
let main argv =
    ...
    printfn "|%-20s|%10s|%10s|%10s|%10s|%10s|%10s|" "Name" "Price" "Open" "Low"
        "High" "UpDown" "UpDownRate"
    stocks
    ...
    |> Seq.iter (fun result ->
           match result with
           | result when result.Data = None ->
               printfn "|%-20s|%10s|%10s|%10s|%10s|%10s|%10s|" result.Stock.Name
                   "_" "_" "_" "_" "_" "_"
           | { StockResult.Stock = stock; StockResult.Data = Some data } ->
               printfn "|%-20s|%10.2f|%10.2f|%10.2f|%10.2f|%10.2f|%10.2f%%|"
                   stock.Name data.Price data.Open data.Low data.High
                   data.UpDown data.UpDownRate
           | _ -> ())
    0
